...
namespace zparsernmsp {
    ...
    bool ZCppParser::expectFunctionDefinition() {
    ...
    void ZCppParser::parse(ZCppTokenizer &tokens) {
    ...
    optional<Token> ZCppParser::expectIdentifier(const string &name) {
    ...
    optional<Token> ZCppParser::expectOperator(const string &name) {
        if (mCurrentToken == mEndToken) { return nullopt; }
    ...
    optional<Type> ZCppParser::expectType() {
        optional<ZCppToken> possibleType = expectIdentifier();
        if (!possibleType) { return nullopt; }

        map<string, Type>::iterator foundType = mTypes.find(possibleType->Text.toCChar());
    ...
    optional<vector<Statement>> ZCppParser::parseFunctionBody() {
        if (!expectOperator("{").has_value()) {
            return nullopt;
        }
    ...
    void ZCppParser::debugPrint() const {
        for (auto funcPair : mFunctions) {
            funcPair.second.debugPrint();
        }
    }

    optional<Statement> ZCppParser::expectOneValue() {
    ...
    optional<Statement> ZCppParser::expectVariableDeclaration() {
    ...
    optional<Statement> ZCppParser::expectFunctionCall() {
        vector<Token>::iterator startToken = mCurrentToken;
    ...
    optional<Statement> ZCppParser::expectWhileLoop() {
    ...
    optional<Statement> ZCppParser::expectStatement() {
    ...
    optional<Statement> ZCppParser::expectExpression() {
    ...
    Statement * ZCppParser::findRightmostStatement(Statement *lhs, size_t rhsPrecedence) {
    ...
    size_t ZCppParser::operatorPrecedence(const string &operatorName) {
